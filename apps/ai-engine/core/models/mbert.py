...
_LOCAL_MODEL_DIR = Path(__file__).parents[3] / "model_artifacts" / "mbert"

_CATEGORIES = ["SCAM", "PHISHING", "ADVERTISEMENT", "FAKE_GIVEAWAY", "SOCIAL_ENGINEERING", "SUSPICIOUS_URL", "OTHER"]
_SPAM_IDX = 1


class MultilingualBERTAdapter(BaseModelAdapter):
    def __init__(self, model_name: str = "bert-base-multilingual-cased", max_length: int = 512) -> None:
    ...
    def load(self) -> None:
        try:
            ...
            logger.info("mBERT loading", extra={"device": self._device, "model": self._model_name})
            ...
            logger.info("mBERT loaded", extra={"source": source, "device": self._device})
        except Exception:
            logger.warning(
                "mBERT failed to load; adapter will return stub predictions. "
